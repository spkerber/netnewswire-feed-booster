...
from netnewswire_feed_booster.example_bundle import build_example_bundle
...
class ExampleBundleTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.manifest_path = self.project_root / "examples" / "starter-bundle.json"

    def test_builds_requested_sources_opml_preview_and_generated_feeds(self) -> None:
        def fake_fetch(url: str, **_kwargs) -> str:
            return BANDCAMP_HTML if url.rstrip("/").endswith("/music") else DIRECT_RSS

        with TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch("netnewswire_feed_booster.example_bundle.fetch_text", side_effect=fake_fetch):
            ...
            preview = (root / "exports" / "starter-preview.html").read_text(encoding="utf-8")
        ...
        self.assertIn("Pitchfork Album Reviews", preview)
        self.assertIn("Bandcamp: Dark Entries Records", preview)
        self.assertIn("Nothing has been imported automatically", preview)
